...
class NoiseGen< float, 2 >
{
	...
	GradientCoherentNoise( ValueType x, ValueType y, int seed = 0, NoiseQuality noiseQuality = QUALITY_STD )
	{
		...
		M::Vector4F		coordV = M::vectorize( x, y, ValueType( 0.0 ), ValueType( 0.0 ) );
		

		// Map the difference between the coordinates of the input value and the
		// coordinates of the cube's outer-lower-left vertex onto an S-curve.
		M::Vector4F		sCoordV;
		M::Vector4F		xy__0 = M::vectorize( ValueType( x0 ), ValueType( y0 ), ValueType( 0.0 ), ValueType( 0.0 ) );
		
		// Calculate distance from the "zero" cube coordinates
		sCoordV = M::subtract( coordV, xy__0 );
		...
		M::Vector4F		xy01 = M::vectorize( ValueType( x1 ), ValueType( x0 ), ValueType( y1 ), ValueType( y0 ) );
		M::Vector4F		__01 = M::vectorize( ValueType( 0.0 ), ValueType( 0.0 ), ValueType( 0.0 ), ValueType( 0.0 ) );
		...
		M::Vector4F		interpV = M::vectorize( xs, xs, xs, xs );
	...
	calculateNoisePartSSELocal0_4( M::Vector4F& coordV, M::Vector4F& xy01, M::Vector4F& __01, int* tableIndices, M::Vector4F& n0 )
	{
		...
		for( int i = 0; i < 4; ++i )
		{
			noise[ i ] = M::subtract( coordV, noise[ i ] );
		}
		...
		// scale noise values
		M::Vector4F		noiseScale = M::vectorizeOne( ValueType( 2.12 ) );
